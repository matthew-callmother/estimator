...
const DEFAULT_GOOGLE_ADS_CONVERSION_NAME = "Sold Estimates (not completed rev)";
...
async function upsertAttributionRows(rows, run) {
  ...
  const existing = await getValues(spreadsheetId, `${quoteSheet(attributionSheet)}!A1:AB`);
  ...
  for (const row of rows) {
    ...
    if (!found) {
      ...
      updates.push({
        range: `${quoteSheet(attributionSheet)}!A${found.rowNumber}:AB${found.rowNumber}`,
        values: [values]
      });
    } else {
      unchanged += 1;
  ...
  for (const chunk of chunks(inserts, 500)) {
    await sheetsRequest(
      `${SHEETS_API}/spreadsheets/${spreadsheetId}/values/${encodeURIComponent(`${quoteSheet(attributionSheet)}!A:AB`)}:append?valueInputOption=RAW&insertDataOption=INSERT_ROWS`,
...
function rowValues(row) {
  return [
    row.recordKey,
    googleAdsTimestamp(row.leadDate),
    row.leadType,
    row.classification,
    row.landingPage,
    row.referrerUrl,
    ...
    row.estimateTotal,
    row.jobRevenue,
    googleAdsTimestamp(row.lastSyncedAt),
    googleAdsConversionName(row)
  ].map((entry) => entry === null || entry === undefined ? "" : entry);
}

function googleAdsConversionName(row) {
  const isSold = String(row.estimateStatus || "").trim().toLowerCase() === "sold";
  const hasValue = row.estimateTotal !== null && row.estimateTotal !== undefined && row.estimateTotal !== "";
  if (!isSold || !hasValue) return "";
  return String(process.env.GOOGLE_ADS_CONVERSION_NAME || DEFAULT_GOOGLE_ADS_CONVERSION_NAME).trim();
}

function syncRowValues(run) {
  return [
    run.runId,
    googleAdsTimestamp(run.startedAt),
    googleAdsTimestamp(run.completedAt),
    run.status,
    googleAdsTimestamp(run.fromUtc),
    googleAdsTimestamp(run.toUtc),
    run.fetched,
    run.uniqueRecords,
    run.inserted,
    run.updated,
...
function googleAdsTimestamp(value) {
